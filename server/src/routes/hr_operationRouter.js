...
} = require('../validators/hr_operationValidator');
...
router.post(
  ...
  authMiddleware,
  validate(hireSchema),
  hrOperationsController.hireEmployee
); // Принять сотрудника на работу
router.put(
  '/salary/:employee_id',
  authMiddleware,
  validate(salaryChangeSchema),
  hrOperationsController.changeSalary
); // Изменить зарплату сотрудника
router.put(
  '/department/:employee_id',
  authMiddleware,
  validate(departmentChangeSchema),
  hrOperationsController.changeDepartment
); // Перевести сотрудника в другой отдел
router.post(
  '/fire/:employee_id',
  authMiddleware,
  hrOperationsController.fireEmployee
); // Уволить сотрудника
